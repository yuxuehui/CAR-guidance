...
class ODESolver(Solver):
    ...
    def compute_likelihood(
        ...
        **model_extras,
        ...
        # ODE may diverge (e.g. guidance weight mismatch), producing NaN trajectories.
        # Return NaN log-probs instead of crashing in log_p0's _validate_sample.
        if torch.isnan(x_source).any():
            nan_log_p = torch.full(
                (x_source.shape[0],), float('nan'), device=x_source.device
            )
            if return_intermediates:
                return sol, nan_log_p
            else:
                return sol[-1], nan_log_p
