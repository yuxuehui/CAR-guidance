...
from experiments.experiments.exp4_mixed import Exp4Mixed
...
def main():
    parser = argparse.ArgumentParser(description='运行实验4：混合能量场Guidance（静态+动态）+ g_cov_a_gm_online')
    parser.add_argument(
        '--config',
        type=str,
        default='experiments/configs/exp4_mixed_gcov.yaml',
        help='配置文件路径'
    )
    parser.add_argument(
        '--success-json',
    ...
    def run_with_immediate_save(test_cases):
        ...
        for idx, case in enumerate(test_cases):
            ...
            try:
                ...
                if hasattr(exp, '_last_static_energy_centers') and exp._last_static_energy_centers is not None:
                    result['energy_centers'] = exp._last_static_energy_centers
                elif hasattr(exp, '_last_energy_centers') and exp._last_energy_centers is not None:
                    result['energy_centers'] = exp._last_energy_centers
                if hasattr(exp, '_last_path_functions') and exp._last_path_functions is not None:
                    result['path_functions'] = 'dynamic_paths'
                ...
                if 'trajectory' in result and 'error' not in result:
                    ...
                    energy_centers = result.get('energy_centers', None)
                    path_functions = getattr(exp, '_last_path_functions', None)
                    ...
                    visualize_trajectory(
                        ...
                        show=False,
                        title=f"Exp4: Mixed Guidance (Demo {idx+1})",
                        ...
                        if hasattr(exp.model, 'guide_model') and hasattr(exp.model.guide_model, 'step_data'):
                            ...
                            if step_data and hasattr(exp, '_last_traj_normalizer'):
                                ...
                                visualize_step_by_step(
                                    ...
                                    show=False,
                                    title=f"Exp4: Mixed Guidance - Step-by-Step (Demo {idx+1})"
                                )
