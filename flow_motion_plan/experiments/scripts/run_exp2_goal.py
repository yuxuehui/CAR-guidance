...
from experiments.experiments.exp2_goal import Exp2Goal
...
def main():
    parser = argparse.ArgumentParser(description='运行实验2：目标吸引Guidance + g_cov_a_gm_online')
    parser.add_argument(
        '--config',
        type=str,
        default='experiments/configs/exp2_goal_gcov.yaml',
        help='配置文件路径'
    )
    parser.add_argument(
        '--success-json',
    ...
    def run_with_immediate_save(test_cases):
        ...
        for idx, case in enumerate(test_cases):
            ...
            try:
                ...
                collision_margin = config.get('evaluation', {}).get('collision_margin', 0.0)
                wall_size = config.get('evaluation', {}).get('wall_size', 1.0)
                ...
                valid_walls = []
                for w in walls_array:
                    w = np.asarray(w, dtype=np.float64)
                    if not (abs(w[0]) < 1e-6 and abs(w[1]) < 1e-6):
                ...
                if len(valid_walls) > 0:
                    for p in traj:
                        ...
                        for w in valid_walls:
                            w = np.asarray(w, dtype=np.float64)
                            half = float(wall_size) / 2.0 + float(collision_margin)
                ...
                if eval_result['reached_goal'] != manual_reached:
                    log_fp.write(f"  ⚠️ 警告: 到达终点评估不一致 (评估: {eval_result['reached_goal']}, 手动: {manual_reached})\n")
                if eval_result['has_collision'] != manual_collision:
                    log_fp.write(f"  ⚠️ 警告: 碰撞检测不一致 (评估: {eval_result['has_collision']}, 手动: {manual_collision})\n")
                log_fp.write("\n")
                log_fp.flush()
                ...
                if 'trajectory' in result and 'error' not in result:
                    ...
                    visualize_trajectory(
                        ...
                        show=False,
                        title=f"Exp2: Goal Guidance (Demo {idx+1})",
                        goal_tol=config.get('evaluation', {}).get('goal_tolerance', 0.3)
                    )
                        ...
                        if hasattr(exp.model, 'guide_model') and hasattr(exp.model.guide_model, 'step_data'):
                            ...
                            if step_data and hasattr(exp, '_last_traj_normalizer'):
                                ...
                                visualize_step_by_step(
                                    ...
                                    show=False,
                                    title=f"Exp2: Goal Guidance - Step-by-Step (Demo {idx+1})"
                                )
