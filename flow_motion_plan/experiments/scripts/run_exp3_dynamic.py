...
from experiments.experiments.exp3_dynamic import Exp3Dynamic
...
def main():
    parser = argparse.ArgumentParser(description='运行实验3：动态障碍物Guidance + g_cov_a_gm_online')
    parser.add_argument(
        '--config',
        type=str,
        default='experiments/configs/exp3_dynamic_gcov.yaml',
        help='配置文件路径'
    )
    parser.add_argument(
        '--success-json',
            ...
            try:
                ...
                if 'trajectory' in result and 'error' not in result:
                    ...
                    visualize_trajectory(
                        ...
                        show=False,
                        title=f"Exp3: Dynamic Guidance (Demo {idx+1})",
                        ...
                        if hasattr(exp.model, 'guide_model') and hasattr(exp.model.guide_model, 'step_data'):
                            ...
                            if step_data and hasattr(exp, '_last_traj_normalizer'):
                                ...
                                visualize_step_by_step(
                                    ...
                                    show=False,
                                    title=f"Exp3: Dynamic Guidance - Step-by-Step (Demo {idx+1})"
                                )
