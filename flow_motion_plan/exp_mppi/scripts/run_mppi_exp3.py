...
def main():
    parser = argparse.ArgumentParser(description='运行MPPI实验3：动态障碍物')
    parser.add_argument('--model', type=str,
                       default='checkpoints/flow_model/best_checkpoint.pth',
                       help='Flow模型检查点路径')
    parser.add_argument('--config', type=str,
                       default='exp_mppi/configs/mppi_exp3_dynamic.yaml',
                       help='MPPI配置文件路径')
    parser.add_argument('--seed', type=int, default=42,
                       help='随机种子')
    parser.add_argument('--success-json', type=str,
                       default='experiments/data/success_trajectories.json',
                       help='成功轨迹 JSON（与 experiments/run_exp3_dynamic 一致，保证同一 demo）')
    ...
    log_file = log_dir / f'mppi_exp3_dynamic_{timestamp}.txt'
    ...
    with open(log_file, 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        f.write("MPPI实验3：动态障碍物 Baseline\n")
        f.write("="*70 + "\n\n")
