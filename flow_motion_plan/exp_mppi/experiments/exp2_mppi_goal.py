...
from experiments.experiments.exp2_goal import select_interfering_energy_centers_from_base_traj
...
class Exp2MPPIGoal:
    ...
    def run(self, test_cases: List[Dict]):
        ...
        print(f"\n{'='*70}")
        print(f"开始运行实验2：目标吸引场干扰 (MPPI Baseline)")
        ...
        for idx, case in enumerate(test_cases):
            ...
            print("  [Step 1] 生成base轨迹用于选择干扰能量中心...")
            ...
            print(f"      干扰能量中心数量: {len(energy_centers)}")
            for i, center in enumerate(energy_centers):
                print(f"      中心{i+1}: {center}, 缩放: {energy_scales[i]} (吸引)")
            ...
            fig = visualize_trajectory(
                ...
                wall_positions=walls,
                energy_centers=energy_centers,
                title=f"MPPI Exp2 - Case {case_id}"
            )
