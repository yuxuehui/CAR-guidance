...
from pfp.data.dataset_maniskill import RobotDatasetManiskill
...
def main():
    ...
    data_path_train = str(repo_root / "data" / "demo_data_pcd_from_three_cameras_small")
    data_path_valid = str(repo_root / "data" / "demo_data_pcd_from_three_cameras_valid")

    seed = 1234
    epochs = 1500
    log_wandb = False
    run_name = "maniskill_train_pcd_from_three_cameras_more_gripper_encoder_goal_pos"

    obs_features_dim = 256

    y_dim = 10
    goal_pos_dim = 128
    x_dim = obs_features_dim + y_dim + goal_pos_dim
    n_obs_steps = 2
    n_pred_steps = 32
    use_ema = True
    ...
    composer_model: ComposerModel = FMPolicy(
        x_dim=x_dim,
        y_dim=y_dim,
        n_obs_steps=n_obs_steps,
        n_pred_steps=n_pred_steps,
    ...
    dataset_train = RobotDatasetManiskill(**dataset_kwargs)
    ...
    dataset_valid = RobotDatasetManiskill(**dataset_kwargs)
