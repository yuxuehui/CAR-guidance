...
class FMPolicy(ComposerModel, BasePolicy):
    ...
    def _norm_data(self, batch: tuple[torch.Tensor, ...]) -> tuple[torch.Tensor, ...]:
        pcd, robot_state_obs, robot_state_pred, goal_pos = batch
        ...
        return pcd, robot_state_obs, robot_state_pred, goal_pos
    ...
    def loss(self, outputs, batch: tuple[torch.Tensor, ...]) -> torch.Tensor:
        ...
        loss_xyz, loss_rot6d, loss_grip = self.calculate_loss(
            pcd, robot_state_obs, robot_state_pred, goal_pos
        )
        loss = (
    ...
    def calculate_loss(
        self, pcd: torch.Tensor, robot_state_obs: torch.Tensor, robot_state_pred: torch.Tensor, goal_pos: torch.Tensor
    ):
