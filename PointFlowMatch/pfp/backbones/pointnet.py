...
class PointNetBackbone(nn.Module):
    def __init__(
        ...
        self.goal_encoder = nn.Sequential(
            nn.Linear(3, 64),
            nn.Mish(),
            nn.Linear(64, goal_encoder_dim),
            nn.Mish()
        )
        self.goal_encoder_dim = goal_encoder_dim
        if use_group_norm:
    ...
    def forward(self, pcd: torch.Tensor, robot_state_obs: torch.Tensor = None, goal_pos: torch.Tensor = None) -> torch.Tensor:
        B = pcd.shape[0]

        pcd = pcd.float().reshape(-1, *pcd.shape[2:])
        robot_state_obs = robot_state_obs.float().reshape(-1, *robot_state_obs.shape[2:])
        goal_pos = goal_pos.float().reshape(-1, *goal_pos.shape[2:])

        pcd = pcd.permute(0, 2, 1)

        encoded_pcd = self.backbone(pcd)
        encoded_goal = self.goal_encoder(goal_pos)
        nx = torch.cat([encoded_pcd, encoded_goal, robot_state_obs], dim=1)
