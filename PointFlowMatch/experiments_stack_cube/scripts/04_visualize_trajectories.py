...
def load_results(results_path: Path):
    if results_path.is_dir():
        ...
        if len(demo_files) == 0:
            raise ValueError(f"目录中没有找到 demo 文件: {results_path}")
        results = []
...
def smooth_trajectory(trajectory, num_points=1000, s=0.0):
    ...
    try:
        x, y, z = clean_traj[:, 0], clean_traj[:, 1], clean_traj[:, 2]
...
def create_trajectory_line(scene, trajectory, cmap_name="turbo", line_width=0.003, alpha=1.0):
    ...
    for i in range(total_points - 1):
        ...
        if np.abs(np.dot(direction, x_axis)) > 0.99:
            rotation = np.array([1, 0, 0, 0]) if np.dot(direction, x_axis) > 0 else np.array([0, 0, 1, 0])
        else:
...
def visualize_trajectory(
    ...
    if "energy_scales" in result_data:
        ...
        energy_scales = [-1.0] * len(energy_centers) if energy_centers else []
...
def main():
    parser = argparse.ArgumentParser(
        description="Stack-Cube 可视化轨迹：重放实验并可视化能量场和轨迹"
    )
    parser.add_argument(
        "--results_path",
        type=str,
        required=True,
        help="实验结果路径：detailed_results.json、actions_summary.json 或 actions/ 目录",
    )
    parser.add_argument(
        "--experiment_name",
        type=str,
        default=None,
        help="实验名称（用于输出文件名，默认从路径推断）",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="输出目录（默认与 results_path 同目录）",
    )
    parser.add_argument(
        "--num_demos",
        type=int,
    ...
    if not results_path.exists():
        print(f"错误: 结果路径不存在: {results_path}")
        return
    ...
    if args.experiment_name:
        ...
        elif results_path.name in ["actions_summary.json", "actions.json", "detailed_results.json"]:
    ...
    print("\n可视化完成！")
    print(f"视频保存在: {output_dir / 'visualization_videos'}")
    print(f"图片保存在: {output_dir / 'visualization_images'}")
