...
from ..guidance import StaticGuidance, GCovGuidance, NoneGuidance

class Exp1StaticGcov(Exp1Static):
    ...
    def _result_to_serializable(self, result: Dict[str, Any]) -> Dict[str, Any]:
        import numpy as np
        out: Dict[str, Any] = {}
    ...
    def run_inference(self, demo: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            initial_cube_pos = base_env.cube.pose.p[0].detach().cpu().numpy()
            initial_ee_pos = get_robot_state(env).detach().cpu()[:3].numpy()
            debug_msg = (
                f"[DEBUG] 演示 {demo_id} 初始状态: seed={seed}\n"
                f"[DEBUG]  cube_pos=[{initial_cube_pos[0]:.4f}, {initial_cube_pos[1]:.4f}, {initial_cube_pos[2]:.4f}]\n"
            ...
            try:
                ...
                if not cube_grasped and is_grasped_now:
                    ...
                    if not first_correction_completed:
                        ...
                        self.logger.info(
                            f"演示 {demo_id} 步骤 {step_count}: Cube被抓取 (z: {initial_cube_z:.3f}->{cube_z:.3f})！"
        ...
        while not done and step_count < max_steps:
            ...
            for i in range(min(num_actions_per_step, n_pred_steps_model)):
                ...
                try:
                    cube_pos = base_env.cube.pose.p[0].detach().cpu().numpy()
                    goal_pos = base_env.goal_site.pose.p[0].detach().cpu().numpy()
                    ...
                    debug_msg = (
                        ...
                        f"动作={action_str}, "
                        f"cube_pos=[{cube_pos[0]:.4f}, {cube_pos[1]:.4f}, {cube_pos[2]:.4f}], "
                        f"goal_pos=[{goal_pos[0]:.4f}, {goal_pos[1]:.4f}, {goal_pos[2]:.4f}], "
        ...
        try:
            final_cube_pos = base_env.cube.pose.p[0].detach().cpu().numpy()
            final_goal_pos = base_env.goal_site.pose.p[0].detach().cpu().numpy()
            final_ee_pos = get_robot_state(env).detach().cpu()[:3].numpy()
            cube_goal_distance = np.linalg.norm(final_cube_pos - final_goal_pos)
            debug_msg = (
                f"[DEBUG] 演示 {demo_id} 最终状态:\n"
                f"[DEBUG]  cube_pos=[{final_cube_pos[0]:.4f}, {final_cube_pos[1]:.4f}, {final_cube_pos[2]:.4f}]\n"
                f"[DEBUG]  goal_pos=[{final_goal_pos[0]:.4f}, {final_goal_pos[1]:.4f}, {final_goal_pos[2]:.4f}]\n"
                f"[DEBUG]  cube到goal的距离: {cube_goal_distance:.4f}米\n"
